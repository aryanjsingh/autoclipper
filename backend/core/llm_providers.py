...
class KiroGatewayProvider(LLMProvider):
    ...
    def __init__(self, api_key: str = "", model_name: str = DEFAULT_KIRO_MODEL, **kwargs):
        ...
        if not self.api_key:
            raise ValueError(
                "未找到Kiro Gateway API密钥。请设置KIRO_GATEWAY_API_KEY，"
                "或在../kiro-gateway/.env中配置PROXY_API_KEY"
            )
        ...
        try:
            ...
            if response.status_code >= 400:
                ...
                try:
                    ...
                    pass
                raise Exception(f"Kiro Gateway调用失败 - Status: {response.status_code}, Message: {error_message}")
            ...
            logger.error(f"Kiro Gateway调用失败: {str(e)}")
            raise
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            logger.error(f"Kiro Gateway连接测试失败: {e}")
            return False
    ...
    def get_available_models(self) -> List[ModelInfo]:
        ...
        return [
            ModelInfo(
                ...
                max_tokens=200000,
                description="Kiro Gateway默认高质量模型"
            ),
            ModelInfo(
