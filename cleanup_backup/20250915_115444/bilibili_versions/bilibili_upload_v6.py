"""
Bilibili Upload Service v6.0
Browser-behavior-based correct implementation
"""
...
class BilibiliUploaderV6:
    """Bilibili Upload Uploader v6.0 - Browser-behavior-based"""
    ...
    async def upload_video(self, video_path: str, metadata: dict, max_retries: int = 3) -> bool:
        """Upload video main flow - Browser-behavior-based"""
        try:
            async with aiohttp.ClientSession() as session:
                ...
                # 3. Chunk upload - Browser-behavior-based
    ...
    async def _chunk_upload_browser_style(self, video_path: str, upload_info: dict, max_retries: int = 3) -> bool:
        """Chunk upload - Browser-behavior-based"""
        try:
            file_size = os.path.getsize(video_path)
            ...
            # Use more complete browser request headers
            headers = {
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
                "Referer": "https://member.bilibili.com/",
                "Origin": "https://member.bilibili.com",
    ...
    async def _merge_chunks_browser_style(self, upload_info: dict) -> bool:
        """Merge chunks - Browser-behavior-based"""
        try:
            # Parse Cookie to get CSRF token
            csrf_token = None
            for cookie in self.cookies.split(';'):
    ...
    async def _submit_video_browser_style(self, upload_info: dict, metadata: dict) -> bool:
        """Submit upload - Browser-behavior-based"""
        try:
            # Parse Cookie to get CSRF token
            csrf_token = None
            for cookie in self.cookies.split(';'):
