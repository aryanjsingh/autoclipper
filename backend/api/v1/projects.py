...
async def upload_files(
    ...
    try:
        ...
        # Generate thumbnail immediately (sync processing)
        try:
            ...
            # Thumbnail generation failure does not affect main flow, will be retried in async task
        
        # Process subtitle file (if user provided)
        srt_path = None
        if srt_file:
        ...
        try:
            ...
            logger.info(f"Project {project_id} async processing task started, Celery task ID: {celery_task.id}")
            
        except Exception as e:
            logger.error(f"Failed to start project {project_id} async processing: {str(e)}")
            # Even if async task fails to start, return project creation success
        ...
        response_data = {
            ...
            "video_path": str(video_path),  # Add video_path field
            "settings": {
                "video_category": video_category or "knowledge",
                "video_file": video_file.filename,
                "srt_file": subtitle_info
        ...
        # Thumbnail will be generated in async task
...
async def get_project(
    ...
    try:
        ...
        # If clips and collections data is needed, load them
        clips_data = None
        ...
        # Create response data with clips and collections
...
async def sync_all_projects_data(
    ...
    """Sync all project data to database"""
    try:
...
async def sync_project_data(
    ...
    """Sync specified project data to database"""
    try:
        from ...services.data_sync_service import DataSyncService
        ...
        if result.get("success"):
            return {
                "message": "Project data sync successful",
                "result": result
            }
        else:
...
async def start_processing(
    ...
    try:
        ...
        # Get video and SRT file paths
        video_path = project.video_path
        srt_path = None
        
        # Get SRT file path from processing_config
        ...
        # If no SRT file path, try to auto-find
        if not srt_path:
            ...
            if srt_file.exists():
                ...
                # SRT file is optional, if not found, set to None
                srt_path = None
        elif not Path(srt_path).exists():
            # If specified SRT file does not exist, try to auto-find
            video_dir = Path(video_path).parent
            srt_file = video_dir / "input.srt"
            if srt_file.exists():
                srt_path = str(srt_file)
...
async def retry_processing(
    ...
    try:
        ...
        # Check project status - allow retry for failed, completed, processing, and pending states
        ...
        # Send WebSocket notification - WebSocket notifications disabled
        # await websocket_service.send_processing_started(
        #     project_id=int(project_id),
        #     message="Restarting processing pipeline"
        # )
        
        # Get file paths and resubmit task
        from ...core.path_utils import get_project_raw_directory
        raw_dir = get_project_raw_directory(project_id)
        video_path = raw_dir / "input.mp4"  # Use standard input.mp4 filename
        srt_path = raw_dir / "input.srt"    # Use standard input.srt filename
        
        # Check if video file exists, try to re-download if not
        if not video_path.exists():
            logger.warning(f"Video file does not exist: {video_path}, attempting re-download")
            
            # Check if project metadata has source URL
            if hasattr(project, 'project_metadata') and project.project_metadata:
                source_url = project.project_metadata.get('source_url')
                if source_url:
                    logger.info(f"Found source URL: {source_url}, starting re-download")
                    
                    # Choose download method based on URL type
                    if 'bilibili.com' in source_url:
                        # Bilibili video re-download
                        ...
                        return {
                            "message": "Video file does not exist, Bilibili video re-download started",
                            "project_id": project_id,
                            "download_task_id": download_task_id,
                            "source_url": source_url
                        }
                    elif 'youtube.com' in source_url or 'youtu.be' in source_url:
                        # YouTube video re-download
                        ...
                        return {
                            "message": "Video file does not exist, YouTube video re-download started",
                            "project_id": project_id,
                            "download_task_id": download_task_id,
                            "source_url": source_url
                        }
                    ...
                    raise HTTPException(status_code=400, detail=f"Video file does not exist and no source URL: {video_path}")
            else:
                raise HTTPException(status_code=400, detail=f"Video file does not exist and no project metadata: {video_path}")
        
        # Subtitle file is optional
        srt_path_str = str(srt_path) if srt_path.exists() else None
        
        # Submit Celery task - use string type project_id
        celery_task = process_video_pipeline.delay(
            project_id=project_id,
            input_video_path=str(video_path),
        ...
        # Send error notification - WebSocket notifications disabled
        # try:
        #     await websocket_service.send_processing_error(
        #         project_id=int(project_id),
        #         error=str(e),
...
async def resume_processing(
    ...
    try:
        ...
        # Call processing service to resume execution
...
async def get_project_logs(
    ...
    """Get project logs."""
    try:
        # Simulated log data, should actually be obtained from log service
        return {
            "logs": [
                {
                ...
                },
                {
                    ...
                    "module": "processing",
                    "level": "INFO",
                    "message": "Step 1: Outline extraction completed"
                },
                {
                    "timestamp": "2025-08-01T13:30:10.000Z",
                    "module": "processing",
                    "level": "INFO",
                    "message": "Step 2: Timeline positioning completed"
                },
                {
...
async def get_import_status(
    ...
    try:
        ...
        # Check for ongoing import tasks
        from ...core.celery_app import celery_app
        
        # More complex task status check logic can be added here
        # Currently simply returns project status
        return {
            "project_id": project_id,
...
async def get_project_file(
    ...
    try:
        ...
        possible_paths = [
            project_root / "raw" / filename,  # Original files
            project_root / "metadata" / filename,  # Metadata files
            project_root / filename,  # Directly in project root
        ]
        ...
        if filename.endswith('.json'):
            # JSON files return data
            with open(file_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
            return data
        else:
            # Other files (like video) return file stream
...
async def get_project_clip(
    ...
    try:
        ...
        # First try to find by clip_id
        video_files = list(clips_dir.glob(f"{clip_id}_*.mp4"))
        
        # If not found, try to find all mp4 files, then match via database
        if not video_files:
...
async def sync_all_projects_from_filesystem(
    ...
    """Sync all project data from file system to database"""
    try:
        ...
        logger.error(f"Failed to sync all project data: {e}")
...
async def reorder_collection_clips(
    ...
    try:
        ...
        logger.error(f"Failed to reorder collection {collection_id} clips: {e}")
...
async def sync_project_from_filesystem(
    ...
    """Sync specified project data from file system to database"""
    try:
        from backend.services.data_sync_service import DataSyncService
        from backend.core.config import get_data_directory
        ...
        return {
            ...
            "message": f"Project {project_id} sync completed"
        }
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Failed to sync project {project_id} data: {e}")
        raise HTTPException(status_code=500, detail=f"Sync failed: {str(e)}")
...
async def generate_collection_video(
    ...
    try:
        ...
        # Get clip ID list from collection
        metadata = getattr(collection, 'collection_metadata', {}) or {}
        clip_ids = metadata.get('clip_ids', [])
        
        if not clip_ids:
            raise HTTPException(status_code=400, detail="Collection has no clips")
        
        # Get clip info, ordered by clip_ids
        ...
        # Get clips in user-adjusted order
        ...
        # Prepare clip video file paths in user-adjusted order
        clips_dir = project_dir / "output" / "clips"
        clip_video_paths = []
        ...
        for clip in ordered_clips:
            if clip.video_path and Path(clip.video_path).exists():
                ...
                # Try to find in clips directory
                possible_paths = [
        ...
        # Generate collection video filename - use collection title as filename
        collection_name = collection.name or f"collection_{collection_id}"
        # Use VideoProcessor's sanitize_filename method to clean filename
        from ...utils.video_processor import VideoProcessor
        safe_name = VideoProcessor.sanitize_filename(collection_name)
        output_filename = f"{safe_name}.mp4"
        output_path = collections_dir / output_filename
        
        # Use VideoProcessor to create collection
        ...
        try:
            ...
            # Extract cover from video (frame at 5 seconds)
...
async def download_project_file(
    project_id: str,
    clip_id: Optional[str] = Query(None, description="Download specified clip"),
    collection_id: Optional[str] = Query(None, description="Download specified collection"),
    db: Session = Depends(get_db),
    project_service: ProjectService = Depends(get_project_service)
):
    """Download project file (clip or collection)"""
    try:
        from fastapi.responses import FileResponse
        from pathlib import Path
