...
const SettingsPage: React.FC = () => {
  ...
  const [selectedProvider, setSelectedProvider] = useState('kiro')
  ...
  const providerConfig = {
    ...
    kiro: {
      ...
      icon: <RobotOutlined />,
      color: '#13c2c2',
      description: '本地Kiro Gateway代理',
      apiKeyField: 'kiro_api_key',
      placeholder: '留空则自动读取 ../kiro-gateway/.env',
  ...
  const handleTestApiKey = async () => {
    const selectedConfig = providerConfig[selectedProvider as keyof typeof providerConfig]
    const apiKey = form.getFieldValue(selectedConfig.apiKeyField)
    const modelName = form.getFieldValue('model_name')
    
    if (!apiKey && !selectedConfig.apiKeyOptional) {
  ...
  const handleProviderChange = (provider: string) => {
    const nextConfig = providerConfig[provider as keyof typeof providerConfig]
    setSelectedProvider(provider)
    form.setFieldsValue({ llm_provider: provider, model_name: nextConfig.defaultModel })
  }
            ...
            <Card title="AI 模型配置" className="settings-card">
              ...
              >
                ...
                <Form.Item
                  ...
                  rules={[
                    {
                      required: !providerConfig[selectedProvider as keyof typeof providerConfig].apiKeyOptional,
                      message: '请输入API密钥'
                    },
                ...
                {selectedProvider === 'kiro' && (
                  <Form.Item
                    label="Kiro Gateway 地址"
                    name="kiro_base_url"
                    className="form-item"
                  >
                    <Input
                      placeholder="留空则使用 ../kiro-gateway/.env 或 http://127.0.0.1:9000/v1"
                      className="settings-input"
                    />
                  </Form.Item>
                )}
              ...
              <Space direction="vertical" size="large" className="instructions-space">
                <div className="instruction-item">
                  ...
                  <Paragraph className="instruction-text">
                    ...
                    <br />• <Text strong>Kiro Gateway</Text>：使用本机父目录中的 kiro-gateway 服务
                  </Paragraph>
                </div>
